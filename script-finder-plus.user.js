// ==UserScript==
// @name            Script Finder+ (Mobile & Performance)
// @name:zh-CN        Script Finder 油猴脚本查找 (手机适配+高性能版)
// @description:zh-CN 适配手机触摸拖动，缩小移动端图标。渲染优先：先显示脚本再异步翻译。记录位置、点击外部关闭。
...
(function () {
    ...
    const getT = (key) => {
        const dict = isChinese ? {
            ...
            Loading: '正在请求数据...', LoadMore: '加载更多', AllLoaded: '到底啦',
            Search: '搜索脚本名称或描述...', Scripts: '脚本查找', Total: '本站脚本总数', Timeout: '请求超时'
        } : {
            ...
            Search: 'Search...', Scripts: 'Scripts', Total: 'Total Scripts', Timeout: 'Timeout'
        }
        return dict[key] || key
    }
        ...
        GM_xmlhttpRequest({
            ...
            onload: (res) => {
                ...
                if (!scripts || scripts.length === 0) {
                    if (page === 1) { hint.innerText = "该域名暂无适配脚本"; hint.style.display = 'block' }
                    return
                }
    ...
    function appendItem(s) {
        ...
        li.innerHTML = `
            <div class="sf-item-card">
                <a class="sf-name" href="${s.url}" target="_blank" style="font-size:17px; font-weight:bold; color:#1e90ff; text-decoration:none;">${s.name}</a>
                ...
                <p class="sf-desc" style="font-size:14px; color:#444; margin:8px 0; line-height:1.4;">${s.desc}</p>
                ...
                <div style="display:flex; flex-wrap:wrap; gap:10px; font-size:13px; color:#666;">
                    <span>👤 ${s.author}</span><span>📥 ${s.installs}</span><span>⭐ ${s.rating}</span>
                </div>
                <a href="https://greasyfork.org/scripts/${s.id}/code/script.user.js" style="display:inline-block; margin-top:8px; background:#28a745; color:#fff; padding:5px 12px; border-radius:4px; text-decoration:none; font-size:13px;">安装 v${s.version}</a>
            </div>`
        list.appendChild(li)
        return li
    }
    ...
    function setupUI() {
        GM_addStyle(`
            ...
            @media screen and (max-width: 768px) {
                scrbutton.sf-main-btn { padding: 8px 16px; font-size: 14px; right: 10px; }
            }
            div.sf-panel {
                display: none; position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%);
                width: 95%; max-width: 700px; background: #fff; border-radius: 12px; z-index: 1000000;
                box-shadow: 0 15px 50px rgba(0,0,0,0.4); padding: 15px; max-height: 80vh; overflow-y: auto;
            }
            @media (prefers-color-scheme: dark) {
                ...
                .sf-info-item { border-bottom: 1px solid #444 !important; }
            }
        `)
        ...
        panel.className = 'sf-panel'; panel.innerHTML = `
            <div style="display:flex; justify-content:space-between; align-items:center; border-bottom:2px solid #1e90ff; padding-bottom:10px;">
                <div style="font-size:18px; font-weight:bold; color:#1e90ff;">Script Finder<span class="sf-total-count" style="font-size:12px; color:#999; font-weight:normal;"></span></div>
                <button class="sf-close" style="border:none; background:none; cursor:pointer; font-size:24px; color:#999;">&times;</button>
            </div>
            <input type="text" class="sf-search" placeholder="${getT('Search')}" style="width:100%; padding:10px; margin:12px 0; border:1px solid #ddd; border-radius:6px; font-size:14px; box-sizing:border-box;">
        ...
        const onMove = (e) => {
            ...
            const nt = Math.max(10, Math.min(startTop + dy, window.innerHeight - 40))
        ...
        const showBtn = (d = 2000) => { button.style.display = 'block'; setTimeout(() => button.style.opacity = '0.9', 10); if (collapsed && !isDragging) startTimer(d) }
